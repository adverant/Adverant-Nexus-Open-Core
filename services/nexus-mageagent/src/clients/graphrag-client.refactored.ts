...
// ============================================================================
// 🔒 SECURITY FIX: Tenant Context Interface
// ============================================================================
...
export class GraphRAGClient {
  ...
  /**
   * Get tenant context headers for GraphRAG requests
   * These headers MUST be included in all requests to ensure multi-tenant isolation
   */
          ...
          try {
            ...
            response = await this.httpClient.post('/api/v2/memory', {
              ...
              metadata: {
                ...
                source: 'mageagent',
                timestamp: new Date().toISOString(),
                // Tenant context is automatically added via request interceptor
              }
            });
  ...
  async recallEpisodes(request: any): Promise<any[]> {
    ...
    try {
      ...
      return response.data.unified_memories || response.data.episodes || [];
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
...
// ❌ REMOVED: Global singleton (breaks multi-tenancy)
