...
export class VoyageModelDiscovery {
  ...
  // CRITICAL: Only include models that actually exist in Voyage AI API
  private readonly modelPatterns = [
    // General models (verified to exist)
    { prefix: 'voyage', version: '3', specialization: 'general' as const },
    { prefix: 'voyage', version: '2', specialization: 'general' as const },

    // Specialized embedding models (verified to exist)
    ...
    // Multimodal embedding model (uses /multimodalembeddings endpoint)
    // Released November 2024, state-of-the-art multimodal embedding
    // Handles interleaved text + images, screenshots, PDFs, tables
  ...
  private async probeKnownModels(discoveredModels: Map<string, VoyageModelInfo>): Promise<void> {
    ...
    const probePromises = this.modelPatterns.map(async (pattern) => {
      ...
      // CRITICAL: Multimodal models use DIFFERENT endpoint than standard embedding models
      // voyage-multimodal-3 requires /multimodalembeddings, not /embeddings
      if (pattern.specialization === 'multimodal') {
        try {
          ...
          if (dimensions) {
            ...
            logger.info(`Discovered multimodal model via probe: ${modelId}`, {
              dimensions,
              endpoint: 'multimodalembeddings',
              note: 'Uses dedicated multimodal endpoint'
            });
            return;
          }
        } catch (error) {
          logger.debug(`Multimodal probe failed for ${modelId}`, { error });
        }
      } else {
        // Standard embedding models use /embeddings endpoint
        try {
          ...
          if (dimensions) {
            ...
            logger.info(`Discovered standard embedding model via probe: ${modelId}`, {
              dimensions,
              endpoint: 'embeddings',
              specialization: pattern.specialization
            });
          ...
          logger.debug(`Standard embedding probe failed for ${modelId}`, { error });
        }
      }
    });
  ...
  private async probeRerankModels(discoveredModels: Map<string, VoyageModelInfo>): Promise<void> {
    ...
    const probePromises = this.rerankModels.map(async (modelId) => {
      try {
        ...
        // VoyageAI rerank returns response.data.data array, not response.data.results
        if (response.data?.data && Array.isArray(response.data.data)) {
          ...
          logger.info(`Discovered rerank model: ${modelId}`, {
            modelReturned: response.data.model,
  ...
  async getModelBySpecialization(specialization: 'general' | 'code' | 'finance' | 'law' | 'multimodal'): Promise<VoyageModelInfo | null> {
    ...
    // Find all EMBEDDING models with this specialization (exclude rerank models)
    const candidates = Array.from(models.values())
      .filter(m =>
        m.specialization === specialization &&
        m.endpoint !== 'rerank'  // Critical: exclude rerank models from embedding selection
      )
      .sort((a, b) => {
