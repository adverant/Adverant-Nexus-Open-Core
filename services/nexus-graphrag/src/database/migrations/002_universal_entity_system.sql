...
CREATE INDEX IF NOT EXISTS idx_universal_entities_hierarchy_path ON graphrag.universal_entities (hierarchy_path text_pattern_ops);
...
CREATE INDEX IF NOT EXISTS idx_entity_relationships_source ON graphrag.entity_relationships(source_entity_id);
CREATE INDEX IF NOT EXISTS idx_entity_relationships_target ON graphrag.entity_relationships(target_entity_id);
CREATE INDEX IF NOT EXISTS idx_entity_relationships_type ON graphrag.entity_relationships(relationship_type);
CREATE INDEX IF NOT EXISTS idx_entity_relationships_bidirectional ON graphrag.entity_relationships(source_entity_id, target_entity_id);
...
-- Removed idx_llm_interactions_recent - WHERE clause with NOW() is not immutable
-- Instead, use the standard idx_llm_interactions_platform index for time-based queries
